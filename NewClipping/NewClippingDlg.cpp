...
void CNewClippingDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT10, Area_eps_sloy);
}
...
BEGIN_MESSAGE_MAP(CNewClippingDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON5, &CNewClippingDlg::eps_sloy)
END_MESSAGE_MAP()
...
std::vector<Paths> res_eps(1);
double epsilon = 10;
double coef = 10;

CBrush Real(RGB(255, 255, 255));
bool obol = 0;
POINT *res_eps_struct;

POINT *make_structure(std::vector<Path> init_vector);
...
void CNewClippingDlg::OnPaint()
{
	...
	{
		CPaintDC dc(this);
		CPen obolochka(PS_SOLID, 1, RGB(0, 0, 127));
		...
		if (obol == 1) {
			...
			dc.Polygon(res_eps_struct, res_eps[0][0].size());
			obol = 0;
		}
		if (DrawCircles == 1) {
			dc.SelectObject(Real);
...
void CNewClippingDlg::Activate()
{
	...
	RadiusSlider.SetPos(100);
	SectorSlider.SetRange(0, 360, 1);
	SectorSlider.SetPos(0);
...
POINT * make_structure(std::vector<Path> init_vector)
{
	POINT * result_structure;
	result_structure = new POINT[init_vector[0].size()];
	for (int i = 0; i < init_vector[0].size(); i++) {
		result_structure[i].x = init_vector[0][i].X / pow(10.0, 6.0);
		result_structure[i].y = init_vector[0][i].Y / pow(10.0, 6.0);
	}
...
std::vector<Path> UnionSector(1);
std::vector<Path> e_sloy(1);
ClipperOffset co;
double area_of_sloy;
...
void CNewClippingDlg::eps_sloy()
{
	//DoClip.Clear();

	epsilon = 100;

	//Выше в функции РеалСектор раскоменировать строки!!!!!!!!!!!!!
	//Если угол альфа равен не 0
	//DoClip.AddPath(RealSector[0], ptSubject, true); //
	//DoClip.AddPath(MirrorSector[0], ptClip, true); //
	//DoClip.Execute(ctUnion, UnionSector, pftNonZero, pftNonZero); //The clipper library syntax 
	
	//Если альфа равен 0
	for (int i = 0; i < RealSector[0].size(); i++) {
		UnionSector[0] << RealSector[0][i];
	}
	for (int i = 0; i < MirrorSector[0].size(); i++) {
		UnionSector[0] << MirrorSector[0][i];
	}


	co.Clear();
	co.AddPath(UnionSector[0], jtRound, etClosedPolygon);
	co.Execute(res_eps[0],  epsilon * pow(10.0, 6.0));
	res_eps_struct = new POINT[res_eps[0][0].size()];
	for (int i = 0; i < res_eps[0][0].size(); i++) {
		res_eps_struct[i].x = res_eps[0][0][i].X / pow(10.0, 6.0);
		res_eps_struct[i].y = res_eps[0][0][i].Y / pow(10.0, 6.0);
	}
	obol = 1;
	area_of_sloy = abs(Area(res_eps[0][0])) / pow(10.0, 12.0);
	area_of_figure = abs(Area(UnionSector[0])) / pow(10.0, 12.0);
	res_area = area_of_sloy - area_of_figure;

	TextForLine.Format(_T("%f"), res_area / 10000);
	Area_eps_sloy.SetWindowTextW(TextForLine);
